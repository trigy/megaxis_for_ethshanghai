...
func (s *SPrompt) GetAllPrompt(ctx context.Context, in *v1.GetAllPromptReq) (*v1.GetAllPromptRes, error) {
	...
	all, err := dao.Prompt.Ctx(ctx).LeftJoin("`promptlike`", "prompt.pid = promptlike.pid").Fields("prompt.*,"+
		...
		Group("prompt.pid").Order("likes desc, id desc").Limit(in.Offset, in.Limit).All()
	if err != nil {
		return nil, err
	}
	res = s.GetResult(ctx, all, 3)
	total := len(res)
	return &v1.GetAllPromptRes{PromptInfos: res, Total: total}, nil
}
...
func (s *SPrompt) GetMyPrompt(ctx context.Context, in *v1.GetMyPromptReq) (*v1.GetMyPromptRes, error) {
	...
	all, err := dao.Prompt.Ctx(ctx).LeftJoin("`promptlike`", "prompt.pid = promptlike.pid").Fields("prompt.*,"+
		" COUNT(CASE WHEN promptlike.like = 1 THEN promptlike.id END) likes, COUNT(CASE WHEN promptlike.like = -1 THEN promptlike.id END) dislikes, promptlike.like attitude").
		Where("prompt.author ", in.Uid).Group("prompt.pid").Order("likes desc, id desc").Limit(in.Offset, in.Limit).All()
	if err != nil {
		return nil, err
	}
...
func (s *SPrompt) GetPromptBought(ctx context.Context, in *v1.GetPromptBoughtReq) (*v1.GetPromptBoughtRes, error) {
	...
	all, err := dao.Prompt.Ctx(ctx).LeftJoin("`promptlike`", "prompt.pid = promptlike.pid").Fields("prompt.*,"+
		" COUNT(CASE WHEN promptlike.like = 1 THEN promptlike.id END) likes, COUNT(CASE WHEN promptlike.like = -1 THEN promptlike.id END) dislikes, promptlike.like attitude").
		WhereIn("prompt.pid", pids).Group("prompt.pid").Order("likes desc, id desc").Limit(in.Offset, in.Limit).All()
	if err != nil {
		return nil, err
	}
...
func (s *SPrompt) GetPromptByKey(ctx context.Context, in *v1.GetPromptByKeyReq) (*v1.GetPromptByKeyRes, error) {
	...
	if in.Key == "" {
		...
		count, _ := dao.Prompt.Ctx(ctx).Count()
		res = s.GetResult(ctx, all, 3)
		...
		all, err := dao.Prompt.Ctx(ctx).LeftJoin("`promptlike`", "prompt.pid = promptlike.pid").Fields("prompt.*,"+
			...
			WhereOrLike("taglist", "%"+in.Key+"%").Group("prompt.pid").Order("likes desc, id desc").Limit(in.Offset, in.Limit).All()
		if err != nil {
			return nil, err
		}
...
func (s *SPrompt) GetPromptByType(ctx context.Context, in *v1.GetPromptByTypeReq) (*v1.GetPromptByTypeRes, error) {
	...
	all, err := dao.Prompt.Ctx(ctx).LeftJoin("`promptlike`", "prompt.pid = promptlike.pid").Fields("prompt.*,"+
		" COUNT(CASE WHEN promptlike.like = 1 THEN promptlike.id END) likes, COUNT(CASE WHEN promptlike.like = -1 THEN promptlike.id END) dislikes").
		Where("prompt.type = ", in.Type).Group("prompt.pid").Order("likes desc, id desc").Limit(in.Offset, in.Limit).All()
	if err != nil {
		return nil, err
	}
	...
	for _, prompt := range all {
		...
		pi := &v1.PromptInfo{
			Prompt: &entity.Prompt{
				...
				CreateTime: create_time.(*gtime.Time),
			},
...
func (s *SPrompt) GetPrompt(ctx context.Context, in *v1.GetPromptReq) (*v1.GetPromptRes, error) {
	...
	all, err := dao.Prompt.Ctx(ctx).LeftJoin("`promptlike`", "prompt.pid = promptlike.pid").Fields("prompt.*,"+
		" COUNT(CASE WHEN promptlike.like = 1 THEN promptlike.id END) likes, COUNT(CASE WHEN promptlike.like = -1 THEN promptlike.id END) dislikes, promptlike.like attitude").
		Where("prompt.pid ", in.Pid).Group("prompt.pid").Order("likes desc, id desc").All()
	if err != nil {
		return nil, err
	}
	if prompt == nil {
